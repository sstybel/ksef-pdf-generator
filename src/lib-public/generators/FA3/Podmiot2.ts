  ...
  if (podmiot2?.JST) {
    result.push(
      createLabelText(
        ...
        i18n.t(
          getValue(podmiot2?.JST)?.toString().trim() === '1' ? 'invoice.subject2.yes' : 'invoice.subject2.no'
        ),
        FormatTyp.Default,
        { marginTop: 8 }
      )
  ...
  if (podmiot2?.GV) {
    result.push(
      createLabelText(
        ...
        i18n.t(
          getValue(podmiot2?.GV)?.toString().trim() === '1' ? 'invoice.subject2.yes' : 'invoice.subject2.no'
        ),
        FormatTyp.Default,
